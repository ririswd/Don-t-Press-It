...
describe("ConfidentialERC20 Tests", function () {
  ...
  beforeEach(async function () {
    ...
    for (const [name, userWallet] of Object.entries(namedWallets)) {
      ...
      if (balanceEth < 0.05) {
        ...
        // Funding is a value transfer — 1 confirmation is fine; CONFIRMATIONS on the mint test is what matters.
  ...
  describe("----------- Minting Tests -----------", function () {
    it("Should mint tokens using plain mint() by owner", async function () {
      ...
      // Use CONFIRMATIONS so we don't read state from a lagging public-RPC replica.
      const mintReceipt = await publicClient.waitForTransactionReceipt({ hash: txHash, confirmations: CONFIRMATIONS });
    ...
    it("Should mint tokens using encryptedMint()", async function () {
      ...
      await publicClient.waitForTransactionReceipt({ hash: txHash, confirmations: CONFIRMATIONS });
